...
// Check the time since the last sensor update.
$timerQuery = $conn->query("
    SELECT TIMESTAMPDIFF(SECOND, MAX(timestamp), NOW()) 
    AS seconds_ago FROM simulation_data
");
$timerRow         = $timerQuery->fetch_assoc();
$secondsSinceLast = $timerRow['seconds_ago'];

// If 5 seconds passed, the FIRST person to load
// the map triggers new data for EVERYONE.
if ($secondsSinceLast === null || $secondsSinceLast >= 5) {
    ...
    if (!empty($sensors)) {
        $stmt = $conn->prepare("
            ...
            (sensor_id, movement_level) VALUES (?, ?)
        ");
...
while ($row = $result->fetch_assoc()) {
    $locations[] = [
        ...
        'risk'           => $row['risk_level'],
        'movement_level' => $row['movement_level'] !== null 
